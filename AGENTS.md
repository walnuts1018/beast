...
- データベースやオブジェクトストレージが漏洩した場合のリスクを考慮し、動画とタグはサーバー側のAES-256-GCMで暗号化して保存します。動画は固定長チャンク単位で暗号化し、APIが認証済みリクエストに必要な範囲だけを復号して返します。端末鍵やクライアント側復号は使用せず、OIDCアカウントによる複数デバイス再生を可能にします。
...
- 開発するときは~/ghq/github.com/walnuts1018/piccaを参考にするといいでしょう。UIとかを真似て欲しいというわけではなくて、GraphQLの設計とかライブラリの選定とか、CIとかローカル開発とかアーキテクチャとかそういう方針などなどで参考にするという意味です。でもライブラリ選定とかはpiccaとなるべく同じ感じにすると良いでしょう。もちろん、必要ならpiccaと変えてもいいですが、とりあえずpiccaと同じようなライブラリを使うことを推奨します。
