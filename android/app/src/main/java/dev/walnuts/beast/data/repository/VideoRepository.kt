...
interface VideoRepository {
    ...
    suspend fun updateTags(videoId: String, tags: List<String>): Video
...
class GraphQlVideoRepository(
    ...
    override suspend fun listVideos(): List<Video> {
        ...
        return transport.execute(GraphQlOperation("ListVideos", VideoGraphQlOperations.listVideos), token)
            .getOrThrow().jsonObject["videos"]!!.jsonArray.map { it.toVideo(apiEndpoint) }
    }

    override suspend fun updateTags(videoId: String, tags: List<String>): Video = executeVideoMutation(
        "UpdateVideoTags",
        "updateVideoTags",
        VideoGraphQlOperations.updateTags,
        buildJsonObject {
            put("id", videoId)
            put("input", buildJsonObject { put("tags", JsonArray(tags.map { JsonPrimitive(it) })) })
        },
        videoId,
    )
...
class PreviewVideoRepository : VideoRepository {
    ...
    private var videos = listOf(
        preview("video-01", VideoStatus.READY, listOf("旅", "夕方"), 4, 5, "1710000000", 623),
        preview("video-02", VideoStatus.READY, listOf("散歩", "街"), 0, null, null, 218),
        preview("video-03", VideoStatus.READY, listOf("料理", "週末"), 9, 4, "1708000000", 496),
        preview("video-04", VideoStatus.ENCODING, listOf("旅行"), 0, null, null, 1280).copy(playbackUrl = null),
        preview("video-05", VideoStatus.READY, listOf("猫", "日常"), 2, 3, "1710200000", 92),
    )
    ...
    override suspend fun updateTags(videoId: String, tags: List<String>): Video = update(videoId) { it.copy(tags = tags) }
    ...
    private fun preview(id: String, status: VideoStatus, tags: List<String>, playCount: Int, rating: Int?, lastPlayedAt: String?, durationSeconds: Long) = Video(
        id = id,
        status = status,
        tags = tags,
        playCount = playCount,
