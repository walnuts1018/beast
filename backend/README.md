...
Shared Keyの公開鍵をGraphQLの`registerSharedKey`で登録した後、`POST /api/videos/upload`へmultipartで`file`、`shared_key_id`、`encrypted_tags`を送信します。サーバーは登録済み公開鍵でEnvelope Encryptionを行い、暗号化メタデータと`objectKey`を返してエンコードジョブをRabbitMQへ発行します。動画が`READY`になった後、返された動画IDの`/api/videos/:id/stream`を取得し、`X-Encryption-Chunk-Size`などのレスポンスメタデータを使ってクライアント側で1MiB単位のAES-GCMチャンクを復号します。
